[README (5).md](https://github.com/user-attachments/files/32460954/README.5.md)
...
A `Dockerfile` is included so this service can be added into the shared
`docker-compose.yml` (Person D owns that file). Build/run it standalone to
test first:

```bash
docker build -t shopsphere-backend-api .
docker run -p 8000:8000 shopsphere-backend-api
```

Snippet for Person D to drop into the shared `docker-compose.yml`:
...
```yaml
  backend-api:
    ...
    ports:
      - "8000:8000"
    # depends_on will matter once real DB calls replace the mocks (Week 2):
    # depends_on:
    #   - mongodb
    #   - cassandra
    #   - redis
